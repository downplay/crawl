...
void bolt::affect_cell()
{
    ...
    if (cell_is_solid(pos()) 
        // Wall affecting beams should still be allowed to
        // XX: Shame to duplicate this check which also happens inside affect_wall
    ...
    // Also allow clouds to be placed on wall monster
    if (!cell_is_solid(pos()) || m)
        affect_ground();
}
...
void bolt::do_fire()
{
    ...
    // Tracks if the *last* cell seen was a wall monster, and therefore
    // treat next cell as solid regardless
