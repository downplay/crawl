...
dice_def lunar_fissure_damage(int pow, bool random)
{
    const int max_dam = 50 + (random ? div_rand_round(pow, 2) : pow / 2);
    return calc_dice(1, max_dam, random);
}

static bool _lunar_can_affect(const actor *agent, const monster *target, bool quiet = true)
{
    return !mons_is_conjured(target->type)
            // One very specific exception to moonlight damage
...
static vector<actor *> _get_lunar_fissure_targets(const actor *agent)
{
    ...
    for (actor_near_iterator ai(agent->pos(), LOS_NO_TRANS);
         ai; ++ai)
    {
        if (ai->is_player() || _lunar_can_affect(agent, ai->as_monster(), true))
        {
 ...
 */
static void _lunar_fissure_effect(actor* agent, int pow, int min_duration, vector<actor *> targets)
{
    // TODO: Would be nice to make some sparks and some moonlight sparkles (from runelight)
    const dice_def dam_dice = lunar_fissure_damage(pow);

    for (actor* target : targets)
    {
        // mprf("%s", target->name(DESC_THE).c_str());
        // Everything is blinded except conjured entities (and caster is handled
        // separately) (why?)
        if (target == agent ||
            (!target->is_player() && mons_is_conjured(target->as_monster()->type)))
            continue;

        // Apply blindness first; will only confuse player (as per other sources of
        // monster-inflicted blindness) if they were not the caster
        const bool hitting_player = target->is_player();
        if (hitting_player)
        {
            target->confuse(agent, min_duration + random2(div_rand_round(pow, 50)));
            // If player was the caster they had LOS reduced when casting the spell
            const int base_dam = dam_dice.roll();
            const int dam = target->apply_ac(base_dam);
            if (dam) 
                target->hurt(agent, dam, BEAM_MMISSILE);
        }
        else
        {
            auto ench = mon_enchant(ENCH_BLIND, 1, agent, (min_duration + random2(div_rand_round(pow, 30))) * BASELINE_DELAY);
            monster *mons = target->as_monster();
            // TODO: Will fail to blind mosters flagged as unblindable but this should be an
            // exception. So I think we need to create a separate enchantment.
            if (mons->add_ench(ench) && you.can_see(*target))
                simple_monster_message(*mons, " is blinded.");

            // Monsters can skip actually hurting friendlies (including themselves)
            if (agent->is_monster() && mons_aligned(agent, target))
                continue;

            // Just don't damage unaffected entities
            if (!_lunar_can_affect(agent, mons, false))
                continue;
            ...
            {
                mprf("%s %s is illuminated with moonlight energy%s",
                    target->name(DESC_THE).c_str(),
                    conjugate_verb("are", hitting_player).c_str(),
                    attack_strength_punctuation(dam).c_str());
            }
            if (agent->is_player())
                _player_hurt_monster(*target->as_monster(), dam, BEAM_MMISSILE);
            else if (dam)
                target->hurt(agent, dam, BEAM_MMISSILE);
        }
    }
...
spret cast_lunar_fissure(int powc, actor &caster, bool fail)
{
    ...
    if (caster.is_player()) {
        ...
        if (you.duration[DUR_LUNAR_FISSURE]) {
            mpr("Performing this again so soon would risk tearing apart"
                   " the very fabric of reality.");
            return spret::abort;
        }

        if (you.duration[DUR_BLINK_COOLDOWN]) {
            mpr("You are too translocationally unstable to form a link"
                   " to the moon.");
            return spret::abort;
        }

        // Moonlight IS technically a verb, although we are being more literal here
        if (stop_attack_prompt(hitfunc, "moonlight", affected))
            return spret::abort;

        fail_check();
        
        // TODO: Could briefly override any shadows effects and *increase* LOS for the flash
        mpr("Everything in sight is bathed in raw moonlight!");
        flash_view_delay(UA_PLAYER, WHITE, 300, &hitfunc);
        mpr("Your eyes, accustomed only to darkness, are blinded by its intensity.");

        // Make a list of valid targets *before* we go and reduce the LOS, otherwise
        // nothing will be in LOS and we won't hit anything...
        vector<actor *> targets = _get_lunar_fissure_targets(&caster);

        // Up to 10 turns at max power
        const int blind_duration = 2 + div_rand_round(powc, 80) + random2(1 + div_rand_round(powc, 50));
        you.increase_duration(DUR_LUNAR_FISSURE, blind_duration);
        you.increase_duration(DUR_BLINK_COOLDOWN, blind_duration);

        // LOS will be reduced here
        update_vision_range();

        _lunar_fissure_effect(&caster, powc, blind_duration, targets);

        // Double contamination for player undead (primarily to punish them for
        // avoiding the malmutations)
        const int undead_mult = you.is_lifeless_undead(true) ? 2 : 1;
        // At lowest power 5000-10000 rads, at max power 10000-30000 (without
        // undead doubling)
        contaminate_player(undead_mult * (5000 + powc * 25)
                           + random2(undead_mult * (5000 + powc * 75)));
        return spret::success;
    }
    else
    {
        monster* mon_caster = caster.as_monster();
        simple_monster_message(*mon_caster,
                               " bathes everything in moonlight!");
        targeter_radius hitfunc(mon_caster, LOS_NO_TRANS);
        flash_view_delay(UA_MONSTER, WHITE, 300, &hitfunc);
        const int blind_duration = 2 + random2(1 + div_rand_round(powc, 30));
        auto ench = mon_enchant(ENCH_BLIND, 1, mon_caster, blind_duration);
        // TODO: Will fail to blind mosters flagged as unblindable but we really
        // want to blind everything. Needs its own ENCH_LUNAR_FISSURE.
        if (mon_caster->add_ench(ench))
            simple_monster_message(*mon_caster, " is blinded.");
        vector<actor *> targets = _get_lunar_fissure_targets(&caster);
        _lunar_fissure_effect(mon_caster, powc, blind_duration, targets);
        return spret::success;
    }
}
