...
static void _flesh_sacrifice(monster& caster, bolt& beam)
{
    ...
    ASSERT(monster);
    mprf("%s starts to look healthier as the flesh sloughs off %s bones.",
         caster.name(DESC_THE).c_str(), monster->name(DESC_ITS).c_str());
        ...
        for (sumcount = 0; sumcount < sumcount2; ++sumcount)
        {
            create_monster(
                mgen_data(MONS_SHIELD_WALL, SAME_ATTITUDE(mons), mons->pos(),
                          mons->foe)
                .set_summoned(mons, spell_cast, duration, god));
        }
        return;
