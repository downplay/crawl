...
void rockfish_disperse_fineff::fire()
{
    ...
    {
        coord_def jpos = find_newmons_square_contiguous(MONS_ROCKFISH, posn, 3, false);
        if (!in_bounds(jpos))
            continue;
        ...
        auto mg = mgen_data(MONS_ROCKFISH, spawn_beh, jpos, foe,
                              MG_DONT_COME | MG_FORBID_BANDS);
        auto fish = create_monster(mg);
        spawned++;
    }

    // If we didn't manage to spawn any, just exit
    if (!spawned)
        return;
